...
impl PropertiesProvider for AntioxidantActivityPropertiesProvider {
    ...
    fn get_description(&self) -> &str { "Mock antioxidant activity scorer" }
    ...
    fn get_available_parameters(&self) -> HashMap<String, ParameterDefinition> {
        ...
        m.insert("temperature".into(), ParameterDefinition { name: "temperature".into(), description: "Assay temperature".into(), data_type: ParameterType::Number, required: false, default_value: Some(Value::Number(25.into())) });
        m.insert("method".into(), ParameterDefinition { name: "method".into(), description: "Method variant".into(), data_type: ParameterType::String, required: false, default_value: Some(Value::String("std".into())) });
        m
    }
    async fn calculate_properties(&self, family: &MoleculeFamily, _parameters: &HashMap<String, Value>) -> Result<Vec<LogPData>, Box<dyn std::error::Error>> {
        Ok(family.molecules.iter().enumerate().map(|(i, m)| LogPData { value: 0.5 + 0.05 * (i as f64) + (m.smiles.len() as f64 * 0.01), source: "antiox_activity_mock".into(), frozen: false, timestamp: Utc::now() }).collect())
    }
}

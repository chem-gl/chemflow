...
impl PropertiesProvider for GenericPhysChemProvider {
    ...
    fn get_description(&self) -> &str { "Mock multi-property physicochemical provider" }
    fn get_supported_properties(&self) -> Vec<String> { self.supported.clone() }
    fn get_available_parameters(&self) -> HashMap<String, ParameterDefinition> {
        let mut m = HashMap::new();
        m.insert("seed".into(), ParameterDefinition { name: "seed".into(), description: "Random seed".into(), data_type: ParameterType::Number, required: false, default_value: Some(Value::Number(42.into())) });
        m.insert("intensity".into(), ParameterDefinition { name: "intensity".into(), description: "Scaling factor".into(), data_type: ParameterType::Number, required: false, default_value: Some(Value::Number(1.into())) });
        m
    }
    async fn calculate_properties(&self, _molecule_family: &MoleculeFamily, parameters: &HashMap<String, Value>) -> Result<Vec<LogPData>, Box<dyn std::error::Error>> {

...
mod more_properties_provider_tests {
    ...
    impl PropertiesProvider for AdvancedProvider {
        fn get_name(&self) -> &str { "AdvancedProvider" }
        fn get_version(&self) -> &str { "2.1.0" }
        fn get_description(&self) -> &str { "Advanced provider for testing" }
        fn get_supported_properties(&self) -> Vec<String> {
            vec!["logp".to_string(), "solubility".to_string()]
        }
        fn get_available_parameters(&self) -> HashMap<String, ParameterDefinition> {
            let mut params = HashMap::new();
            params.insert("temperature".to_string(), ParameterDefinition {
            ...
            params.insert("method".to_string(), ParameterDefinition {

...
impl MoleculeProvider for AntioxidantSeedProvider {
    ...
    fn get_description(&self) -> &str {
        "Mock antioxidant reference molecules"
    }
    fn get_available_parameters(&self) -> HashMap<String, ParameterDefinition> {
        let mut m = HashMap::new();
        m.insert("include_phenolics".into(),
                 ParameterDefinition { name: "include_phenolics".into(),
                                       description: "Include phenolic seeds".into(),
                                       data_type: ParameterType::Boolean,
                                       required: false,
                                       default_value: Some(Value::Bool(true)) });
        m.insert("extra_seeds".into(),
                 ParameterDefinition { name: "extra_seeds".into(),
                                       description: "Additional SMILES".into(),
    ...
    async fn get_molecule_family(&self, parameters: &HashMap<String, Value>) -> Result<MoleculeFamily, Box<dyn std::error::Error>> {
        ...
        if let Some(Value::Array(extra)) = parameters.get("extra_seeds") {
            for v in extra {
                if let Some(smi) = v.as_str() {
                    fam.molecules.push(Molecule::from_smiles(smi.to_string())?);
                }
            }
        }

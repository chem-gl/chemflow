...
use crate::providers::data::trait_dataprovider::{DataProvider, DataParameterDefinition, DataParameterType};
...
impl DataProvider for AntioxidantAggregateProvider {
    ...
    fn get_description(&self) -> &str { "Aggregate antioxidant stats" }
    fn get_available_parameters(&self) -> HashMap<String, DataParameterDefinition> {
        let mut m = HashMap::new();
        m.insert("min_score".into(), DataParameterDefinition { name: "min_score".into(), description: "Minimum score filter".into(), data_type: DataParameterType::Number, required: false, default_value: Some(Value::Number(0.into())) });
        m
    }
        ...
        for fam in families {
            if let Some(prop) = fam.get_property("radical_scavenging_score") {
                for v in &prop.values {
                    if v.value >= min { values.push(v.value); }
                }
            }
        }
...
mod tests {
    use super::*;
    use crate::data::family::{MoleculeFamily, FamilyProperty, ProviderReference};

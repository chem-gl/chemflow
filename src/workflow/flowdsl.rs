//! DSL simplificada para ejecutar un flujo con pasos ordenados y branching automático
//! Ejemplo de uso:
...
use crate::workflow::step::{MoleculeAcquisitionStep, PropertiesCalculationStep, DataAggregationStep};
...
impl<'a> FlowSession<'a> {
    pub fn new(manager: &'a mut WorkflowManager) -> Self {
        ...
        Self { manager, families: Vec::new(), executed_order: Vec::new(), last_step_ids: HashMap::new(), last_params_hash: HashMap::new() }
    }

    fn require(&self, required: &[&'static str]) -> Result<(), String> {
        for r in required { if !self.executed_order.contains(r) { return Err(format!("Paso requerido '{r}' no ejecutado aún")); } }
        Ok(())
    }
    ...
    fn maybe_branch(&mut self, logical: &'static str, params: &HashMap<String, Value>) {
        ...
        if let Some(prev_hash) = self.last_params_hash.get(logical) {
            if prev_hash != &hash { // cambio de parámetros -> branch manual desde el último step de ese logical
                if let Some(prev_step_id) = self.last_step_ids.get(logical) { self.manager.create_branch(*prev_step_id); }
            }
        }
    ...
    pub async fn step1_acquire(&mut self, count: u32) -> Result<Uuid, Box<dyn std::error::Error>> {
        ...
        let step = MoleculeAcquisitionStep { id: Uuid::new_v4(), name: "Step1:Acquire".into(), description: "Adquiere moléculas base".into(), provider_name: "test_molecule".into(), parameters: params.clone() };
    ...
    pub async fn step2_logp(&mut self, method: &str) -> Result<Uuid, Box<dyn std::error::Error>> {
        ...
        let step = PropertiesCalculationStep { id: Uuid::new_v4(), name: format!("Step2:LogP({method})"), description: "Calcula LogP".into(), provider_name: "test_properties".into(), property_name: "logp".into(), parameters: params.clone() };
        let out = self.manager.execute_step(&step, self.families.clone(), params).await?;
        self.families = out.families.clone();
        if !self.executed_order.contains(&"step2") { self.executed_order.push("step2"); }
    ...
    pub async fn step3_aggregate(&mut self) -> Result<Uuid, Box<dyn std::error::Error>> {
        ...
        let step = DataAggregationStep { id: Uuid::new_v4(), name: "Step3:Aggregate".into(), description: "Agrega propiedades".into(), provider_name: "antiox_aggregate".into(), result_key: "agg".into(), parameters: HashMap::new() };
        let _out = self.manager.execute_step(&step, self.families.clone(), params).await?;
        if !self.executed_order.contains(&"step3") { self.executed_order.push("step3"); }
        self.last_step_ids.insert("step3", step.id);
        Ok(step.id)
    }

    pub fn current_families(&self) -> &Vec<MoleculeFamily> { &self.families }
}

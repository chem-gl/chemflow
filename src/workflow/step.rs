...
fn validate_parameters(provided: &HashMap<String, Value>,
...
fn validate_prop_parameters(provided: &HashMap<String, Value>,
                            definitions: &HashMap<String,
                                     crate::providers::properties::trait_properties::ParameterDefinition>)
                            -> Result<HashMap<String, Value>, String> {
    let mut result = provided.clone();
    for (k, def) in definitions {
        if !result.contains_key(k) {
...
fn validate_data_parameters(provided: &HashMap<String, Value>,
                            definitions: &HashMap<String,
                                     crate::providers::data::trait_dataprovider::DataParameterDefinition>)
                            -> Result<HashMap<String, Value>, String> {
    let mut result = provided.clone();
    for (k, def) in definitions {
        if !result.contains_key(k) {
...
/// Step que permite adquirir moléculas desde múltiples proveedores y
/// unificarlas en una sola familia (deduplicando por inchikey). Cada proveedor
/// puede tener su propio set de parámetros. Se registra un ProviderReference
/// por cada proveedor usado.
    ...
    async fn execute(&self,
        ...
        for pname in &self.provider_names {
            if let Some(provider) = molecule_providers.get(pname) {
                ...
                let validated =
                    validate_parameters(&provided, &defs).map_err(|e| format!("Param validation failed for {pname}: {e}"))?;
                let fam = provider.get_molecule_family(&validated).await?;
                for m in fam.molecules {
                    if seen.insert(m.inchikey.clone()) {
                        unified.molecules.push(m);
                    }
                }
                unified.parameters.insert(format!("provider_{}_molecule_count", pname),
                                          serde_json::json!(unified.molecules.len()));
        ...
        for m in &unified.molecules {
            ordered.insert(m.inchikey.clone());
        }
        unified.parameters
               .insert("unified_molecule_count".into(), serde_json::json!(unified.molecules.len()));
        ...
        Ok(StepOutput { families: vec![unified.clone()],
                        results: HashMap::new(),
                        execution_info:
                            StepExecutionInfo { step_id: self.id,
                                                step_name: self.name.clone(),
                                                step_description: self.description.clone(),
                                                parameters: params_map.clone(),
                                                parameter_hash:
                                                    Some(crate::database::repository::compute_sorted_hash(&params_map)),
                                                providers_used,
...
impl WorkflowStep for MoleculeAcquisitionStep {
    ...
    async fn execute(&self,
        ...
        let validated =
            validate_parameters(&self.parameters, &param_defs).map_err(|e| format!("Parameter validation failed: {e}"))?;
        // 4. Ejecutar proveedor para construir familia base (origen de la trazabilidad)
        //    y calcular hash canónico consistente.
        ...
        Ok(StepOutput { families: vec![family],
                        ...
                        execution_info:
                            StepExecutionInfo { step_id: self.id,
                                                ...
                                                parameter_hash:
                                                    Some(crate::database::repository::compute_sorted_hash(&validated)),
                                                providers_used: vec![ProviderReference { provider_type:
                                                                                             "molecule".to_string(),
                                                                                         provider_name:
...
impl WorkflowStep for MultiPropertiesStep {
    ...
    async fn execute(&self,
        ...
        // Ejecutar cada spec contra todas las familias de salida (mutándolas de forma
        // controlada)
        for spec in &self.specs {
            if let Some(provider) = prop_providers.get(&spec.provider) {
                let param_defs = provider.get_available_parameters();
                let validated =
                    validate_prop_parameters(&spec.parameters, &param_defs).map_err(|e| {
                                                                               format!("Param validation failed for {}: {e}",
                                                                                       spec.provider)
                                                                           })?;
                for fam in &mut output_families {
                    let vals = provider.calculate_properties(fam, &validated).await?;
                    fam.add_property(spec.property.clone(),
                                     vals,
                                     ProviderReference { provider_type: "properties".into(),
                ...
                providers_used.push(ProviderReference { provider_type: "properties".into(),
                                                        provider_name: spec.provider.clone(),
                                                        provider_version: provider.get_version().to_string(),
                                                        execution_parameters: validated.clone(),
                                                        execution_id: Uuid::new_v4() });
            } // si falta el provider simplemente se ignora (podríamos devolver
            // error si se prefiere)
        }
        // Parámetros serializados (incluyendo parámetros individuales por spec)
        let specs_json: Vec<Value> = self.specs
                                         .iter()
                                         .map(|s| {
                                             serde_json::json!({
                                                 "provider": s.provider,
        ...
        Ok(StepOutput { families: output_families,
                        ...
                        execution_info: StepExecutionInfo { step_id: self.id,
                                                            ...
                                                            parent_step_id: None,
                                                            branch_from_step_id: None,
                                                            input_family_ids: input.families
                                                                                   .iter()
                                                                                   .map(|f| f.id)
                                                                                   .collect(),
                                                            input_snapshot:
                                                                Some(build_input_snapshot(&input.families)),
...
impl WorkflowStep for DataAggregationStep {
    ...
    async fn execute(&self,
        ...
        let defs = provider.get_available_parameters();
        let validated =
            validate_data_parameters(&self.parameters, &defs).map_err(|e| {
                                                                 format!("Data provider parameter validation failed: {e}")
                                                             })?;
...
impl WorkflowStep for FilterStep {
    ...
    async fn execute(&self,
        ...
        for fam in &input.families {
            if let Some(prop) = fam.get_property(&self.property) {
                ...
                if let Some(min_v) = self.min {
                    pass &= prop.values.iter().any(|v| v.value >= min_v);
                }
                if let Some(max_v) = self.max {
                    pass &= prop.values.iter().any(|v| v.value <= max_v);
                }
                if pass {
                    let mut cloned = fam.clone();
                    cloned.id = Uuid::new_v4();
                    cloned.parameters
                          .insert("filtered_from".into(), serde_json::json!(fam.id.to_string()));
                    out.push(cloned);
                }
            }
        }
        ...
        if let Some(m) = self.min {
            params.insert("min".into(), serde_json::json!(m));
        }
        if let Some(m) = self.max {
            params.insert("max".into(), serde_json::json!(m));
        }
        Ok(StepOutput { families: out,
                        results: HashMap::new(),
                        execution_info:
                            StepExecutionInfo { step_id: self.id,
                                                step_name: self.name.clone(),
                                                ...
                                                parameter_hash:
                                                    Some(crate::database::repository::compute_sorted_hash(&params)),
...
impl WorkflowStep for PropertiesCalculationStep {
    ...
    async fn execute(&self,
        ...
        let validated =
            validate_prop_parameters(&self.parameters, &param_defs).map_err(|e| {
...
mod tests {
    ...
    async fn test_molecule_acquisition_step_execute() {
        ...
        mol_providers.insert("test_molecule".to_string(),
    ...
    async fn test_properties_calculation_step_execute() {
        ...
        props_providers.insert("test_properties".to_string(),
        ...
        family.molecules
              .push(Molecule::new("K".to_string(), "CC".to_string(), "I".to_string(), None));
    ...
    async fn test_filter_step() {
        ...
        // add dummy molecule
        family.molecules
              .push(Molecule::new("IK".into(), "CC".into(), "IK".into(), None));
        // manually add property
        let data = vec![crate::data::types::LogPData { value: 2.5,
                                                       source: "calc".into(),
                                                       frozen: false,
                                                       timestamp: chrono::Utc::now() }];
        family.add_property("logp",
                            data,
                            ProviderReference { provider_type: "properties".into(),
                                                ...
                                                execution_id: Uuid::new_v4() },
                            None);
        let step = FilterStep { id: Uuid::new_v4(),
                                name: "filter".into(),
                                description: "filter desc".into(),
                                property: "logp".into(),
                                min: Some(2.0),
                                max: Some(3.0) };
        let input = StepInput { families: vec![family],
                                parameters: HashMap::new() };
        let out = step.execute(input, &HashMap::new(), &HashMap::new(), &HashMap::new())
                      .await
                      .unwrap();

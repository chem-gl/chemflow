...
pub async fn create_pool() -> Result<sqlx::Pool<sqlx::Postgres>, sqlx::Error> {
    match PgPoolOptions::new()
        ...
        Err(sqlx::Error::Database(db_err)) if db_err.code().as_deref() == Some("3D000") => {
            ...
            if let Err(e) = ensure_database_exists(&CONFIG.database.url).await { return Err(e); }
...
/// Ensures the target database exists by connecting to the 'postgres' maintenance DB and issuing CREATE DATABASE.
async fn ensure_database_exists(full_url: &str) -> Result<(), sqlx::Error> {
    // Very lightweight URL parsing: split at last '/' to isolate db name (ignore query params for now)
    ...
    if db_name.is_empty() { return Ok(()); }
    // Build admin URL using 'postgres' maintenance DB (fallback to original if already points there)
    ...
    if let Ok(admin_pool) = PgPoolOptions::new().max_connections(1).connect(&admin_url).await {
        // Issue CREATE DATABASE IF NOT EXISTS (Postgres lacks IF NOT EXISTS for CREATE DATABASE pre-15; emulate)
        // We check pg_database first.
        let exists: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM pg_database WHERE datname = $1")
            .bind(&db_name)
            .fetch_one(&admin_pool)
            .await?;
        if exists.0 == 0 {
            // Safe identifier quoting minimal (no special chars assumed). For safety, refuse suspicious names.

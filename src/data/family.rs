...
pub struct MoleculeFamily {
    ...
    /// Map property name -> property entry (values + proveedores y steps históricos)
...
impl MoleculeFamily {
    ...
    pub fn add_property(&mut self, property_name: impl Into<String>, data: Vec<LogPData>, provider_reference: ProviderReference, step_id: Option<Uuid>) {
        ...
        self.properties.entry(key)
            .and_modify(|existing| {
                existing.values.extend(data.clone());
                existing.providers.push(provider_reference.clone());
                if let Some(s) = step_id { existing.originating_steps.push(s); }
            })
            .or_insert_with(|| FamilyProperty { values: data, providers: vec![provider_reference.clone()], originating_steps: step_id.into_iter().collect() });
        if !self.frozen { self.recompute_hash(); }
    }
    ...
    pub fn freeze(&mut self) {
        ...
        for prop in self.properties.values_mut() { for v in &mut prop.values { v.frozen = true; } }
        self.recompute_hash();
    }
    /// Recalcula el hash canónico de la familia basándose en moléculas, parámetros y nombres de propiedades.

...
use crate::workflow::step::{WorkflowStep, StepInput, StepOutput, StepExecutionInfo, StepStatus};
use crate::data::family::MoleculeFamily;
use async_trait::async_trait;
use uuid::Uuid;

// Dummy step that emits a result to test result_type persistence
struct AggregationLikeStep { id: Uuid }

#[async_trait]
impl WorkflowStep for AggregationLikeStep {
    fn get_id(&self) -> Uuid { self.id }
    fn get_name(&self) -> &str { "agg" }
    fn get_description(&self) -> &str { "agg test" }
    fn get_required_input_types(&self) -> Vec<String> { vec![] }
    fn get_output_types(&self) -> Vec<String> { vec!["aggregation_result".into()] }
    ...
    _d: &HashMap<String, Box<dyn crate::providers::data::trait_dataprovider::DataProvider>>,
        ...
        Ok(StepOutput { families: vec![], results, execution_info: StepExecutionInfo {
            step_id: self.id,
...
async fn test_postgres_migrations_and_result_type() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if db_url.is_empty() { eprintln!("DATABASE_URL not set; skipping integration test"); return Ok(()); }

    let pool = sqlx::PgPool::connect(&db_url).await?;
    // Run pending migrations (simple approach: execute each file in order 000*.sql)
    // Assumes tests run from workspace root
    for mig in [
        "0001_init.sql","0002_relationships.sql","0003_properties_and_results.sql",
        "0004_molecules.sql","0006_parameter_hash_and_freeze.sql","0007_step_results_type.sql","0008_providers_used_gin_index.sql"
    ] { let sql = std::fs::read_to_string(format!("migrations/{mig}"))?; for stmt in sql.split(";\n") { let s = stmt.trim(); if !s.is_empty() { let _ = sqlx::query(s).execute(&pool).await; } } }

    let repo = WorkflowExecutionRepository::with_pool(pool).await;
    // Create a dummy family and persist
    let mut fam = MoleculeFamily::new("Fam".into(), None); fam.recompute_hash();
    ...
    if let Some(pool) = &repo.clone().pool { // access internal field via clone
        let row = sqlx::query("SELECT result_type FROM workflow_step_results WHERE step_id = $1 AND result_key='aggregation'")
            .bind(out.execution_info.step_id)
            .fetch_one(pool)
            .await?;
        let rt: String = row.try_get("result_type")?; assert_eq!(rt, "aggregation");
    }
    Ok(())
}

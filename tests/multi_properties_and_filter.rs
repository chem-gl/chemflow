...
async fn test_multi_properties_and_filter_branch_tree() {
    ...
    let out_multi = manager.execute_step(&step_multi, families, HashMap::new())
                           .await
                           .expect("multi props step");
    assert!(!out_multi.families.is_empty());
    // Verificamos que la propiedad agregada exista
    let fam_after = &out_multi.families[0];
    assert!(fam_after.get_property("logp").is_some(),
            "logp property missing after multi props step");
    ...
    let filter = FilterStep { id: Uuid::new_v4(),
                              ...
                              min: Some(1.0),
                              max: None };
    let out_filter = manager.execute_step(&filter, out_multi.families.clone(), HashMap::new())
                            .await
                            .expect("filter step");
    ...
    assert!(out_filter.families
                      .iter()
                      .any(|f| f.parameters.get("filtered_from").is_some()));
...
async fn test_multi_molecule_acquisition_step() {
    // Mock two molecule providers using existing test provider (register twice with
    // different keys)
    use chemflow_rust::providers::molecule::implementations::test_provider::TestMoleculeProvider;
    let repo = WorkflowExecutionRepository::new(true);
    let mut mols: HashMap<String, Box<dyn chemflow_rust::providers::molecule::traitmolecule::MoleculeProvider>> =
        HashMap::new();
    ...
    // Step
    let multi = MultiMoleculeAcquisitionStep { id: Uuid::new_v4(),
                                               name: "multi_acq".into(),
                                               description: "multi provider acquisition".into(),
                                               provider_names: vec!["prov_a".into(), "prov_b".into()],
                                               parameters_per_provider: HashMap::new() };
    // Execute directly (bypassing manager to test step logic)
    let out = multi.execute(chemflow_rust::workflow::step::StepInput { families: Vec::new(),
                                                                       parameters: HashMap::new() },
                            &mols,
                            &HashMap::new(),
                            &HashMap::new())

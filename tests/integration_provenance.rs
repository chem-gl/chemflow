...
use chemflow_rust::workflow::step::{WorkflowStep, StepInput, StepOutput, StepExecutionInfo, StepStatus};
...
struct PropStep { id: Uuid, name: &'static str }

#[async_trait]
impl WorkflowStep for PropStep {
    fn get_id(&self) -> Uuid { self.id }
    fn get_name(&self) -> &str { self.name }
    fn get_description(&self) -> &str { "prop step" }
    ...
    async fn execute(&self,
        ...
        for fam in &mut input.families {
            let provider_ref = ProviderReference {
                provider_type: "prop".into(),
                provider_name: self.name.into(),
                provider_version: "1.0".into(),
                execution_parameters: HashMap::new(),
                execution_id: self.id,
            };
            fam.add_property("logp", vec![chemflow_rust::data::types::LogPData { value: 1.0, source: self.name.into(), frozen: false, timestamp: Utc::now() }], provider_ref, Some(self.id));
        }
        Ok(StepOutput { families: input.families.clone(), results: HashMap::new(), execution_info: StepExecutionInfo {
            step_id: self.id,
            parameters: input.parameters.clone(),
            parameter_hash: Some(chemflow_rust::database::repository::compute_sorted_hash(&input.parameters)),
            providers_used: Vec::new(),
            start_time: Utc::now(), end_time: Utc::now(), status: StepStatus::Completed,
            root_execution_id: Uuid::new_v4(), parent_step_id: None, branch_from_step_id: None, input_family_ids: input.families.iter().map(|f| f.id).collect(),
        }})
    }
}
...
async fn test_multi_provider_and_autobranch() {
    ...
    let mut params1 = HashMap::new(); params1.insert("X".into(), serde_json::json!(1));
    ...
    let mut params2 = HashMap::new(); params2.insert("X".into(), serde_json::json!(1));
    ...
    let mut params3 = HashMap::new(); params3.insert("X".into(), serde_json::json!(2));

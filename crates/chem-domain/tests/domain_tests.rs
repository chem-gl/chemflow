use chem_domain::{Molecule, MoleculeFamily, MolecularProperty, DomainError};
use serde_json::json;
...
fn test_molecular_property_hash_deterministic() {
	...
	let val = json!({"a": 1});
	let prop1 = MolecularProperty::new(&m, "name", val.clone(), Some("u".to_string()), Some("q".to_string()), true, None);

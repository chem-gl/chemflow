...
fn test_molecular_property_list_equality() {
	// Two lists with the same molecules, values, quality, preferred flag, and metadata should compare equal
	let mol = Molecule::new_molecule_with_smiles("CCO").unwrap();
	let meta = json!({"source": "test"});
	let props1 = vec![
		MolecularProperty::new(&mol, "prop", 1, None, true, meta.clone()),

...
fn test_family_property_and_family_operations() -> Result<(), DomainError> {
	...
	let fprop1 = FamilyProperty::new(&family, "average_logP", 2.5_f64, Some("high".to_string()), true, metadata.clone())?;
	let fprop2 = FamilyProperty::new(&family, "average_logP", 2.5_f64, Some("high".to_string()), true, metadata.clone())?;

...
impl<P: ConnectionProvider> EventStore for PgEventStore<P> {
    ...
    debug!("append_kind:start flow_id={flow_id} kind={}", kind_variant_name(&kind));
                    ...
                    match self.provider.connection() {
                        Ok(mut conn2) => {
                            for h in outputs {
                                ...
                                if
                                    let Err(e) = diesel
                                        ::insert_into(workflow_step_artifacts::table)
                                        .values(&row)
                                        .on_conflict_do_nothing()
                                        .execute(&mut conn2)
                                {
                                    error!(
                                        "artifact insert error hash={h} seq={} err={:?}",
                                        inserted.0,
                                        e
                                    );
                                }
                        ...
                        Err(e) =>
                            error!("artifact connection error seq={} err={:?}", inserted.0, e),
                    }
                }
            }
        }
        #[cfg(feature = "no-artifact-insert")]
        {
            if let FlowEventKind::StepFinished { outputs, .. } = &kind {
                debug!(
                    "artifact insertion skipped by feature no-artifact-insert outputs={}",
                    outputs.len()
                );
            }
        }
        ...
        debug!(
            "append_kind:done flow_id={flow_id} seq={} kind={}",
            ev.seq,
...
pub fn build_pool(
    ...
    if validated_min > validated_max {
        eprintln!(
            "WARN: min_size > max_size ({} > {}), ajustando min=max",
            validated_min,
            validated_max
        );
    }
...
pub fn build_pool_with_migrations(
    database_url: &str,
    min: u32,
    max: u32
) -> Result<PgPool, PersistenceError> {

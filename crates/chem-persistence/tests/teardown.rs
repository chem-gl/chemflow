...
// Si este test (en loop) nunca segfault y otros sí, el crash está en rutas
// adicionales (event/artifact) o múltiple creación. Ejecutar: for i in {1..50};
// do cargo test -q -p chem-persistence --test teardown -- --test-threads=1 ||
// break; done
#[test]
fn explicit_pool_clear_teardown() {
    ...
    eprintln!("state_before: connections={} idle={}",
              state_before.connections, state_before.idle_connections);
    drop(pool);
    eprintln!("teardown test end marker");
    if std::env::var("EXIT_EARLY").is_ok() {
        std::process::exit(0);
    }
}

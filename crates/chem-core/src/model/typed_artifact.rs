...
//! núcleo agnóstico. Permite describir artefactos con un tipo de datos concreto
//! (T) y validaciones. No introduce semántica de dominio; se basa en generics y
//! serde.
...
/// Implementado por tipos de datos que quieren exponerse como artifacts
/// seguros.
pub trait ArtifactSpec: Sized + Serialize + DeserializeOwned + Clone {
    ...
    fn from_artifact(a: &Artifact) -> Result<Self, ArtifactDecodeError> {
        ...
        if let Value::Object(map) = &mut cloned {
            // Si el struct define el campo lo ignorará; si no, preferimos dejarlo presente
            // (serde ignora extra) – conservamos.
            let _ = map; // placeholder
        }
        let decoded: Self = serde_json::from_value(cloned).map_err(|e| ArtifactDecodeError::Deserialize(e.to_string()))?;
        decoded.validate().map_err(ArtifactDecodeError::Validation)?;
        Ok(decoded)
    }
}

/// Adaptador genérico para un artifact tipado ya decodificado (útil en runtimes
/// polimórficos).
...
impl<T: ArtifactSpec + Clone> TypedArtifact<T> {
    pub fn new(inner: T) -> Self {
        Self { raw: inner.clone().into_artifact(),
               inner }
    }

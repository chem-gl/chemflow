...
// Re-export macros for convenience (they're already exported via
// #[macro_export]) pub use step::macros::{typed_artifact, typed_step};
...
mod tests {
    ...
    fn branch_in_memory_copy_creates_branch_and_marks_parent() {
        ...
        let branch_steps: Vec<Box<dyn StepDefinition>> =
            vec![Box::new(SourceStep::new()), Box::new(AStep::new()), Box::new(BStep::new()),];

        let branch_id = parent_engine.create_branch_from_steps(parent_flow_id, branch_steps, "a")
        ...
        assert!(branch_events.iter()
                             .any(|e| matches!(e.kind, FlowEventKind::FlowInitialized { .. })),
                "branch should have FlowInitialized");
        assert!(branch_events.iter()
                             .any(|e| matches!(&e.kind, FlowEventKind::StepFinished { step_id, .. } if step_id == "a")),
                "branch should contain StepFinished for 'a'");
    ...
    fn branch_with_modified_step_produces_different_output() {
        ...
        let parent_a_fp = parent_events.iter()
                                       .find_map(|e| match &e.kind {
                                           FlowEventKind::StepFinished { step_id, fingerprint, .. } if step_id == "a" => {
        ...
        // Crear una rama usando BranchBuilder directamente para evitar la lógica de
        // copia automática
        let branch_steps: Vec<Box<dyn StepDefinition>> = vec![Box::new(SourceStep::new()),
                                                              Box::new(AStepModified::new()),
                                                              Box::new(BStep::new()),];
        let def = build_flow_definition_auto(branch_steps);

        let mut builder = engine.branch_builder(parent_flow_id, def, "a", Some("modified".to_string()))
                                .expect("branch builder");
        ...
        assert!(branch_events.iter()
                             .any(|e| matches!(e.kind, FlowEventKind::FlowInitialized { .. })),
                "branch should have FlowInitialized");
        let branch_a_fp = branch_events.iter()
                                       .find_map(|e| match &e.kind {
                                           FlowEventKind::StepFinished { step_id, fingerprint, .. } if step_id == "a" => {
                                               Some(fingerprint.clone())
                                           }
        ...
        assert_ne!(parent_a_fp, branch_a_fp,
                   "branch 'a' fingerprint should differ from parent after modification");
    ...
    fn branch_change_metadata_at_step2() {
        ...
        // Preparar una definición identica para la rama (podría obtenerse de otra
        // fuente)
        let steps: Vec<Box<dyn StepDefinition>> =
            vec![Box::new(SourceStep::new()), Box::new(AStep::new()), Box::new(BStep::new()),];
        ...
        // Crear y almacenar un artifact con metadata distinta que queremos que use la
        // rama
        let art = Artifact::new_unhashed(ArtifactKind::GenericJson,
                                         json!({"value": "a", "meta": "branch-mod", "schema_version": 1}),
                                         Some(json!({"note": "modified in branch"})));
    ...
    fn branch_create_by_index_example() {
        ...
        let branch_id = engine.create_branch_from_steps_at_index(parent_flow_id, steps, 1)
                              .expect("create branch by index");

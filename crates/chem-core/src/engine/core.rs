          ...
          R: FlowRepository
{
    ...
    /// Inyectores de parámetros (no usados en este archivo, pero parte del
    /// API).
    ...
    /// Definición por defecto (opcional) que permite ejecutar `run()` y
    /// `next()`.
          ...
          R: FlowRepository
{
    ...
    /// actuales del flujo (incluyendo la posible inserción de
    /// `FlowInitialized`).
    ...
    pub fn create_branch_from_steps(&mut self,
                                    parent_flow_id: Uuid,
    ...
    pub fn create_branch_from_steps_at_index(&mut self,
                                             parent_flow_id: Uuid,
                                             steps: Vec<Box<dyn StepDefinition>>,
    ...
    /// Conveniencia: crear y ejecutar una rama a partir de un único
    /// `StepDefinition`.
    ///
    ...
    pub fn create_branch_with_step(&mut self,
                                   parent_flow_id: Uuid,
    ...
    pub fn create_branch_with_step_at_index(&mut self,
                                            parent_flow_id: Uuid,
                                            step: Box<dyn StepDefinition>,
                                            from_step_index: usize)
                                            -> Result<Uuid, CoreEngineError> {
    ...
    /// Ejecuta el flujo completo usando la definición por defecto y devuelve el
    /// `flow_id`.
    ...
    /// Ejecuta un paso del flujo especificado por `flow_id` usando
    /// `definition`.
    ///
    ...
    pub fn branch(&mut self,
        ...
        let _ = self.event_store.append_kind(parent_flow_id,
                                             FlowEventKind::BranchCreated { branch_id,
                                                                            parent_flow_id,
    ...
    ///
    /// Además, durante la copia de eventos valida que los `outputs`
    /// referenciados por `StepFinished` existan en el `artifact_store`; si
    /// falta algún artifact se retorna `CoreEngineError::StorageError`.
    pub fn branch_by_index(&mut self,
                           parent_flow_id: Uuid,
                           definition: &FlowDefinition,
                           from_step_index: usize,
                           divergence_params_hash: Option<String>)
        ...
        let created_from_step_id = definition.steps
                                             .get(from_step_index)
    ...
    pub fn branch_builder<'a>(&'a mut self,
                              parent_flow_id: Uuid,
                              definition: FlowDefinition,
                              from_step_id: &str,
                              divergence_params_hash: Option<String>)
        ...
        Ok(BranchBuilder { engine: self,
                           branch_id,
                           definition })
    }

    /// Versión del builder que acepta un índice de step.
    pub fn branch_builder_by_index<'a>(&'a mut self,
                                       parent_flow_id: Uuid,
                                       definition: FlowDefinition,
                                       from_step_index: usize,
                                       divergence_params_hash: Option<String>)
          ...
          R: FlowRepository
{
    ...
    /// Inserta un evento arbitrario en la rama y devuelve `self` para
    /// encadenar.
    ...
    /// Añade un override de parámetros para un step concreto usando
    /// `StepSignal`.
    pub fn override_step_params(&mut self, step_index: usize, step_id: &str, params: serde_json::Value) -> &mut Self {
        let _ = self.engine.event_store.append_kind(self.branch_id,

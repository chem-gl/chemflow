...
use crate::model::{Artifact, StepFingerprintInput, ExecutionContext, ArtifactSpec, TypedArtifact};
...
/// Wrapper ergonómico que fija `flow_id` y `definition` para evitar repetir parámetros.
...
impl<E: EventStore, R: FlowRepository> FlowEngine<E, R> {
    ...
    /// Nota: en el modo in-memory existe una API ergonómica `FlowEngine::new(definition)`.
    /// Para la versión genérica use `new_with_stores`.
    pub fn new_with_stores(event_store: E, repository: R) -> Self {
        Self { event_store, repository, artifact_store: HashMap::new(), default_definition: None, default_flow_id: None, default_flow_name: None }
    }

    /// Crea un builder genérico para armar un flujo tipado paso a paso con stores personalizados.
    /// Enforcea en tiempo de compilación la compatibilidad de IO entre steps adyacentes.
    ...
    /// Genera además un `flow_id` por defecto, habilitando `*_default_flow()` sin configuración extra.
    pub fn new_with_definition(event_store: E, repository: R, definition: crate::repo::FlowDefinition) -> Self {
        Self { event_store, repository, artifact_store: HashMap::new(), default_definition: Some(definition), default_flow_id: Some(Uuid::new_v4()), default_flow_name: None }
    }
    ...
    /// Igual que `new_with_steps`, pero además define un `flow_id` generado y asigna un nombre.
    pub fn new_with_steps_named(event_store: E, repository: R, flow_name: impl Into<String>, steps: Vec<Box<dyn crate::step::StepDefinition>>) -> Self {
    ...
    pub fn set_default_definition(&mut self, definition: crate::repo::FlowDefinition) { self.default_definition = Some(definition); }
    ...
    pub fn ensure_default_flow_id(&mut self) -> Uuid {
        if self.default_flow_id.is_none() { self.default_flow_id = Some(Uuid::new_v4()); }
        self.default_flow_id.unwrap()
    }

    /// Fija explícitamente un `flow_id` por defecto.
    pub fn set_default_flow_id(&mut self, flow_id: Uuid) { self.default_flow_id = Some(flow_id); }

    /// Fija o actualiza el nombre descriptivo del flow por defecto.
    pub fn set_default_flow_name(&mut self, name: impl Into<String>) { self.default_flow_name = Some(name.into()); }

    /// Obtiene el `flow_id` por defecto si está configurado.
    pub fn default_flow_id(&self) -> Option<Uuid> { self.default_flow_id }

    /// Obtiene el nombre del flow por defecto si está configurado.
    pub fn default_flow_name(&self) -> Option<&str> { self.default_flow_name.as_deref() }

    /// Crea un contexto para un `flow_id` y `definition` específicos.
    /// Este wrapper expone métodos que no requieren volver a pasar `flow_id` ni `definition`.
    pub fn with_flow<'a>(&'a mut self, flow_id: Uuid, definition: &'a FlowDefinition) -> FlowCtx<'a, E, R> {
        FlowCtx { engine: self, flow_id, definition }
    }

    // Nota: No se expone `with_default_flow` para evitar aliasing de préstamos (&mut self con &self.definition).
    ...
    pub fn get_typed_artifact<T: ArtifactSpec + Clone>(&self, hash: &str) -> Option<Result<TypedArtifact<T>, crate::model::ArtifactDecodeError>> {
        self.artifact_store.get(hash).map(|a| TypedArtifact::<T>::decode(a))
    }

    /// Devuelve el último evento `StepFinished` para `step_id` en el `flow_id` dado.
    ///
    /// Notas de diseño y determinismo:
    /// - Los eventos se consultan del `EventStore` en memoria (o persistente en otras implementaciones)
    ///   y se escanean en orden inverso para hallar el más reciente.
    ...
    pub fn last_step_finished_event(&self, flow_id: Uuid, step_id: &str) -> Option<crate::event::FlowEvent> {
        // Obtenemos una copia local de la secuencia de eventos y buscamos desde el final.
        let events = self.event_store.list(flow_id);
        events
            .into_iter()
            .rev()
    ...
    ///
    /// Conveniente para recuperar rápidamente artifacts generados por un step sin
    /// tener que navegar manualmente por la lista de eventos.
    pub fn last_step_output_hashes(&self, flow_id: Uuid, step_id: &str) -> Option<Vec<String>> {
        self
            .last_step_finished_event(flow_id, step_id)
            .and_then(|e| match e.kind { FlowEventKind::StepFinished { outputs, .. } => Some(outputs), _ => None })
    }

    /// Recupera el PRIMER artifact de salida del último `StepFinished` de `step_id`
    /// y lo decodifica como tipo fuerte `T`.
    ///
    /// Recomendado para el patrón pipeline (F2) donde cada step emite 0..1 artifacts
    /// encadenados. Si hay múltiples outputs, se toma el primero por convención.
    ///
    /// Devuelve:
    /// - `None` si el step no tiene `StepFinished` o no produjo outputs.
    /// - `Some(Ok(TypedArtifact<T>))` si la decodificación fue exitosa (kind / versión / validación OK).
    /// - `Some(Err(_))` si hubo un error de decodificación tipada.
    pub fn last_step_output_typed_for<T: ArtifactSpec + Clone>(&self, flow_id: Uuid, step_id: &str)
        -> Option<Result<TypedArtifact<T>, crate::model::ArtifactDecodeError>>
    {
        let first_hash = self
            .last_step_output_hashes(flow_id, step_id)
            .and_then(|v| {
                // Mantener comportamiento predecible: usamos el primer hash según el orden registrado.
                v.into_iter().next()
            });
        match first_hash {
    ...
    pub fn last_event(&self, flow_id: Uuid) -> Option<crate::event::FlowEvent> {
        self.event_store.list(flow_id).into_iter().rev().next()
    }
    ...
    pub fn flow_fingerprint_for(&self, flow_id: Uuid) -> Option<String> {
        self.event_store
            ...
            .into_iter()
            .rev()
            .find_map(|e| match e.kind { FlowEventKind::FlowCompleted { flow_fingerprint } => Some(flow_fingerprint), _ => None })
    }
    ...
    pub fn last_step_fingerprint(&self, flow_id: Uuid, step_id: &str) -> Option<String> {
        self.event_store
            ...
            .into_iter()
            .rev()
            .find_map(|e| match e.kind { FlowEventKind::StepFinished { step_id: sid, fingerprint, .. } if sid == step_id => Some(fingerprint), _ => None })
    }

    /// Devuelve la sub-secuencia de eventos asociados a un `step_id` (Started/Finished/Failed/Signal), en orden.
    ...
    /// Representa la secuencia de eventos como una lista compacta de variantes "I,S,F,X,G,C".
    /// Conveniente para asserts de forma en tests.
    ...
    pub fn last_output_hash(&self, flow_id: Uuid, step_id: &str) -> Option<String> {
        self.last_step_output_hashes(flow_id, step_id).and_then(|v| v.into_iter().next())
    }

    /// Ejecuta `next` repetidamente hasta completar el flujo o encontrar un error terminal.
    /// Útil para ejemplos y tests donde se quiere un run end-to-end con una sola llamada.
        ...
        loop {
            ...
            match self.next_with(flow_id, definition) {
                Ok(()) => {
                    ...
                    if events_after == events_before { break; }
                    // Si ya está completo, terminamos igualmente.
                    if self.is_flow_completed_successfully(flow_id, definition) { break; }
                }
    ...
    /// Igual que `next`, pero usa la definición por defecto configurada en el engine.
    /// Paniquea si no hay definición por defecto.
    pub fn next_default(&mut self, flow_id: Uuid) -> Result<(), CoreEngineError> {
        // Mover temporalmente la definición fuera para evitar préstamos simultáneos (& y &mut).
    ...
    /// Igual que `next_default`, pero usando/eligiendo internamente un `flow_id` por defecto.
    ...
    pub fn last_step_output<T: ArtifactSpec + Clone>(&self, step_id: &str)
        -> Option<Result<TypedArtifact<T>, crate::model::ArtifactDecodeError>>
    {
        self.default_flow_id.and_then(|id| self.last_step_output_typed_for::<T>(id, step_id))
    }
}

// -------------------------------------------------------------
// API ergonómica (especialización) para InMemoryEventStore + InMemoryFlowRepository
// Ruta preferida: new(definition) + next() sin parámetros.
// -------------------------------------------------------------
impl FlowEngine<crate::event::InMemoryEventStore, crate::repo::InMemoryFlowRepository> {
    ...
    pub fn new() -> EngineBuilderInit<crate::event::InMemoryEventStore, crate::repo::InMemoryFlowRepository> {
        EngineBuilderInit { event_store: crate::event::InMemoryEventStore::default(), repository: crate::repo::InMemoryFlowRepository::new() }
    }

    /// Alternativa: crear un motor directamente desde una definición (con flow_id generado).
    ...
    /// Lecturas sin necesidad de pasar `flow_id`.
    pub fn events(&self) -> Option<Vec<crate::event::FlowEvent>> { self.events_default() }
    pub fn event_variants(&self) -> Option<Vec<&'static str>> { self.event_variants_default() }
    pub fn flow_fingerprint(&self) -> Option<String> { self.flow_fingerprint_default() }
    pub fn last_step_output_typed<T: ArtifactSpec + Clone>(&self, step_id: &str)
        -> Option<Result<TypedArtifact<T>, crate::model::ArtifactDecodeError>>
...
impl<'a, E: EventStore, R: FlowRepository> FlowCtx<'a, E, R> {
    ...
    pub fn next(&mut self) -> Result<(), CoreEngineError> { self.engine.next_with(self.flow_id, self.definition) }

    /// Ejecuta hasta completar el flujo (o hasta error terminal).
    pub fn run_to_end(&mut self) -> Result<(), CoreEngineError> { self.engine.run_to_end_with(self.flow_id, self.definition) }

    /// Eventos del flujo.
    pub fn events(&self) -> Vec<crate::event::FlowEvent> { self.engine.events_for(self.flow_id) }

    /// Secuencia compacta de variantes (I,S,F,X,G,C).
    pub fn event_variants(&self) -> Vec<&'static str> { self.engine.event_variants_for(self.flow_id) }

    /// Fingerprint agregado si el flujo está completado.
    pub fn flow_fingerprint(&self) -> Option<String> { self.engine.flow_fingerprint_for(self.flow_id) }

    /// Último output (primer hash) de un step, decodificado como tipo fuerte `T`.
    pub fn last_step_output_typed<T: ArtifactSpec + Clone>(&self, step_id: &str)
        -> Option<Result<TypedArtifact<T>, crate::model::ArtifactDecodeError>>
    { self.engine.last_step_output_typed_for::<T>(self.flow_id, step_id) }

    /// Hash del primer output del último StepFinished de `step_id`.
    pub fn last_output_hash(&self, step_id: &str) -> Option<String> { self.engine.last_output_hash(self.flow_id, step_id) }

    /// Eventos asociados a un step.
    pub fn step_events(&self, step_id: &str) -> Vec<crate::event::FlowEvent> { self.engine.step_events(self.flow_id, step_id) }

    /// Fingerprint del último StepFinished para `step_id`.
    pub fn last_step_fingerprint(&self, step_id: &str) -> Option<String> { self.engine.last_step_fingerprint(self.flow_id, step_id) }
}
...
use crate::step::{StepDefinition, TypedStep, SameAs};
...
impl<E: EventStore, R: FlowRepository> EngineBuilderInit<E, R> {
        ...
        S: TypedStep + 'static,
    {
        ...
        let mut steps: Vec<Box<dyn StepDefinition>> = Vec::new();
        steps.push(Box::new(step));
        EngineBuilder { event_store: self.event_store, repository: self.repository, steps, _out: PhantomData::<S::Output> }
    }
...
impl<S: TypedStep + 'static, E: EventStore, R: FlowRepository> EngineBuilder<S, E, R> {
        ...
        <N as TypedStep>::Input: SameAs<<S as TypedStep>::Output>,
    {
        ...
        EngineBuilder::<N, E, R> { event_store: self.event_store, repository: self.repository, steps: self.steps, _out: PhantomData }
    }

    /// Alias en camelCase solicitado: addStep
    #[allow(non_snake_case)]
    pub fn addStep<N>(self, next: N) -> EngineBuilder<N, E, R>
    where
        N: TypedStep + 'static,
        <N as TypedStep>::Input: SameAs<<S as TypedStep>::Output>,
    { self.add_step(next) }

    /// Construye el FlowEngine con la definición por defecto y flow_id generado.
    pub fn build(self) -> FlowEngine<E, R> {
        ...
        debug_assert!(
            matches!(definition.steps.first().map(|s| s.kind()), Some(crate::step::StepKind::Source)),
...
mod tests {
    ...
    e1.next_with(flow_id, &def1).unwrap(); e1.next_with(flow_id, &def1).unwrap();
        // Run 2
    ...
    e2.next_with(flow_id, &def2).unwrap(); e2.next_with(flow_id, &def2).unwrap();
        // Run 3
    ...
    e3.next_with(flow_id, &def3).unwrap(); e3.next_with(flow_id, &def3).unwrap();
    ...
    fn all_step_fingerprints_equal_across_three_runs() {
        ...
        let run = |flow_id| {
            ...
            e.next_with(flow_id, &def).unwrap(); e.next_with(flow_id, &def).unwrap();
    ...
    engine2.next_with(flow_id, &definition2).unwrap(); engine2.next_with(flow_id, &definition2).unwrap();
    ...
    for _ in 0..ids.len() { engine.next_with(flow_id, &definition).unwrap(); }
        let events = engine.event_store.list(flow_id);
    ...
    e1.next_with(flow_id, &def).unwrap(); e1.next_with(flow_id, &def).unwrap();
